...
function bindDocEvent() {
  ...
  $('#template')
    .mouseenter(function () {
      if ($('#docs>div').length == 0 || !switchStatus) return;
      $('#toolBox').show();
    })
    .mouseleave(function () {
      $('#toolBox').hide();
    ...
    if (!docList[e.id].chapters) {
      sdk.getChapters({ elId: e.id, docId: e.info.docId }).then(function (res) {
        ...
        $.each(res.chapters, function (index, item) {
          txt += '<ul class="title-list"><li class="title"><span>' + item.slideIndex + '<i>' + item.title + '</i></span></li>';
          $.each(item.sub, function (key, value) {
            txt += '<li><span>' + item.slideIndex + '-' + value.stepIndex + '<img src="./images/icon-charles.png" alt="" srcset="" />' + value.title + '</span></li>';
          });
          txt += '</ul>';
        });
        txt += '</div>';
